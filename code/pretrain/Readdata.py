...
class Readdata:
    ...
    def __init__(self,sample_size,llb,front_or_back):
        ...
        for (i, value) in enumerate(range(sample_size)):
            if front_or_back=="FRONT":
                filename = os.path.join(f"./front/exp_data_2/front_exp_{i+1}.txt") # python 从0 开始呢
            elif front_or_back=="BACK":
                filename=os.path.join(f"./back/exp_data_2/back_exp_{i+1}.txt")
    ...
    def ini_state_fun(self):
        beta=pd.read_csv("front/exp_data_2/beta.txt",header=None).values
        ocen=pd.read_csv("front/exp_data_2/ocen.txt",header=None).values
